<template>
  <section class="home-cta">
    <div class="cta-container">
      <h2 class="cta-headline">Start building local-first today.</h2>
      <p class="cta-sub">
        Free, open-source, Apache 2.0 licensed. One API across browser, Node.js,
        and React Native — no cloud required.
      </p>
      <div class="cta-actions">
        <a href="/introduction" class="btn btn-primary btn-lg">Read the Docs</a>
        <a
          href="https://demo-web.taladb.dev/"
          class="btn btn-secondary btn-lg"
          target="_blank"
          rel="noopener"
        >
          Web Demo →
        </a>
        <a
          href="https://appetize.io/app/b_ugmjhjghdkgnjux4lzkepvsfma"
          class="btn btn-secondary btn-lg"
          target="_blank"
          rel="noopener"
        >
          Mobile Demo →
        </a>
      </div>
      <div class="cta-links">
        <a href="https://github.com/tala-sh/taladb" target="_blank" rel="noopener">GitHub</a>
        <span>·</span>
        <a href="https://www.npmjs.com/package/taladb" target="_blank" rel="noopener">npm</a>
        <span>·</span>
        <a href="/roadmap">Roadmap</a>
        <span>·</span>
        <a href="https://github.com/sponsors/tala-sh" target="_blank" rel="noopener">Support TalaDB</a>
      </div>
      <p class="cta-sponsor-note">
        TalaDB is maintained by one person.
        <a href="https://github.com/sponsors/tala-sh" target="_blank" rel="noopener">Sponsoring on GitHub</a>
        directly funds continued development.
      </p>
    </div>
  </section>
</template>

<script setup lang="ts">
</script>
